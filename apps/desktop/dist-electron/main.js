            ...
            exec$f("locate bin/postgres", (error, stdout) => {
              if (!error) {
                ...
                if (postgresqlBin.length) {
                  exec$f(postgresqlBin[postgresqlBin.length - 1] + " -V", (error2, stdout2) => {
                    if (!error2) {
...
async function pollOnce() {
  ...
  try {
    ...
    for (const win2 of BrowserWindow.getAllWindows()) {
      if (!win2.isDestroyed()) {
        win2.webContents.send("system:metrics", metrics);
      }
    }
  } catch {
  } finally {
...
async function collectMetrics() {
  const [cpuInfo, loadInfo, memInfo, graphicsInfo, osInfo2, timeInfo, backend] = await Promise.all([
  ...
  const platform = normalizePlatform(osInfo2.platform);
  return {
    cpu: {
    ...
    system: {
      platform,
      arch: osInfo2.arch || process.arch,
...
function normalizeGpu(graphicsInfo) {
  var _a;
  const controller = (_a = graphicsInfo.controllers) == null ? void 0 : _a[0];
  if (!controller) {
    return null;
  }
  ...
  const usage = utilization !== void 0 ? clampPercent(utilization) : vramTotal && vramUsed ? clampPercent(vramUsed / vramTotal * 100) : void 0;
  return {
...
async function fetchBackendHealth() {
  ...
  if (apiResult.status === "fulfilled" && apiResult.value.ok) {
    ...
    if (data == null ? void 0 : data.model_default) {
      model = data.model_default;
    }
    if ((data == null ? void 0 : data.ollama) === "ok") {
      ollamaStatus = "ok";
    }
  }
  if (ollamaResult.status === "fulfilled" && ollamaResult.value.ok) {
    ollamaStatus = "ok";
    const data = ollamaResult.value.data;
    const firstModel = (_b = (_a = data == null ? void 0 : data.models) == null ? void 0 : _a[0]) == null ? void 0 : _b.name;
    if (!model && firstModel) {
      model = firstModel;
    }
  }
